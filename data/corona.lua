local coronas = {}
local coronaTxt = dxCreateTexture("data/coronaps2.png")
...
function drawCorona(x, y, z, size, color, type)
    ...
    if getScreenFromWorldPosition(x, y, z) and dist < CONFIG.DRAW_DIST then
...
addEventHandler("onClientPreRender", root, function()
    ...
    for i=1,#coronas do
        ...
        if dist < CONFIG.DRAW_DIST then
			local pos = v.pos
			if getScreenFromWorldPosition(pos) then
				local distRatio = dist/400
				local alpha = distRatio < 1 and distRatio or 1
        
                if alpha > CONFIG.NEAR_CLIP_DIST then
    ...
    --print("render in", getTickCount()-tick)
end)
